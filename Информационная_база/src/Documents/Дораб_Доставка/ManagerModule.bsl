 ...
 #Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	 ...
	 Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
		 ...
		 КомандаПечати.Идентификатор = "Дораб_Доставка";
		 КомандаПечати.Представление = НСтр("ru = 'Доставка'");
		 КомандаПечати.Порядок = 5;
		 
		 // Анкета клиента
		 КомандаПечати = КомандыПечати.Добавить();
		 КомандаПечати.Идентификатор = "Дораб_Анкета";
		 КомандаПечати.Представление = НСтр("ru = 'Анкета клиента'");
		 КомандаПечати.Порядок = 5;
		 
		 // Транспортная накладная
		 КомандаПечати = КомандыПечати.Добавить();
		 КомандаПечати.Идентификатор = "Дораб_ТранспортнаяНакладная";
		 КомандаПечати.Представление = НСтр("ru = 'Транспорнтная накладная'");
		 КомандаПечати.Порядок = 5;    
		 
		 // Печать Договора доставки в формате docx, в Word
		 КомандаПечати = КомандыПечати.Добавить();
		 КомандаПечати.Идентификатор = "Дораб_ДоговорНаДоставку";
		 КомандаПечати.Представление = НСтр("ru = 'Договор на доставку'");
		 КомандаПечати.Порядок = 5;
		 
		 // Комплект документов
		 КомандаПечати = КомандыПечати.Добавить();
		 КомандаПечати.Идентификатор = "Дораб_Доставка,Дораб_Анкета,Дораб_ТранспортнаяНакладная";
	 ...
	 Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
		 ...
		 // Доставка
		 ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "Дораб_Доставка");
		 Если ПечатнаяФорма <> Неопределено Тогда
			 ...
			 ПечатнаяФорма.ПолныйПутьКМакету = "Документ.Дораб_Доставка.ПФ_MXL_Доставка";
		 КонецЕсли;
		 
		 // Анкета клиента
		 ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "Дораб_Анкета");
		 Если ПечатнаяФорма <> Неопределено Тогда
		 ...
		 // Транспортная накладная
		 ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "Дораб_ТранспортнаяНакладная");
		 Если ПечатнаяФорма <> Неопределено Тогда
			 ...
			 ПечатнаяФорма.ПолныйПутьКМакету = "Документ.Дораб_Доставка.ПФ_MXL_ТранспортнаяНакладная";
		 КонецЕсли;   
		 
		 //ПечатьДоговора доставки
		 ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "Дораб_ДоговорНаДоставку");
		 Если ПечатнаяФорма <> Неопределено Тогда
	 ...
	 Функция ПечатьДоставки(МассивОбъектов, ОбъектыПечати)
		 ...
		 Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.Дораб_Доставка.ПФ_MXL_Доставка");
	 ...
	 Функция ПечатьАнкеты(МассивОбъектов, ОбъектыПечати) 
		 ...
		 ТабличныйДокумент = Новый ТабличныйДокумент;
		 ТабличныйДокумент.КлючПараметровПечати = "Дораб_Анкета";
	 ...
	 Функция ПечатьТранспортнойНакладной(МассивОбъектов, ОбъектыПечати)
		 ...
		 Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.Дораб_Доставка.ПФ_MXL_ТранспортнаяНакладная");
	 ...
	 Функция НапечататьДоговорDOCX(МассивОбъектов)
		 ...
		 МакетДокумента = УправлениеПечатью.МакетПечатнойФормы("Документ.Дораб_Доставка.ПФ_Docx_ДоговорНаДоставку");  //Тип макета - Двоичные данные
	 ...
	 Процедура ВывестиЗаголовокДоставки(ДанныеДокументов, ТабличныйДокумент, Макет) //Доставка
		 ...
		 ОбластьШапкиДоставки.Рисунки.QRкод.Картинка = ГенерацияШтрихкода(ДанныеДокументов);
	 ...
	 Функция ДанныеДляПечатиДоговора(МассивОбъектов) 
		 ...
		 Запрос.Текст =  "ВЫБРАТЬ
		 |	Дораб_Доставка.Ссылка КАК Ссылка,
		 |	Дораб_Доставка.ВерсияДанных КАК ВерсияДанных,
		 |	Дораб_Доставка.ПометкаУдаления КАК ПометкаУдаления,
		 |	Дораб_Доставка.Номер КАК Номер,
		 |	Дораб_Доставка.Дата КАК Дата,
		 |	Дораб_Доставка.Проведен КАК Проведен,
		 |	Дораб_Доставка.Организация КАК Организация,
		 |	Дораб_Доставка.Контрагент КАК Контрагент,
		 |	Дораб_Доставка.Договор КАК Договор,
		 |	Дораб_Доставка.АдресДоставки КАК АдресДоставки,
		 ...
		 |	Дораб_Доставка.Ответственный КАК Ответственный,
		 |	Дораб_Доставка.Комментарий КАК Комментарий,
		 |	Дораб_Доставка.ЕстьФайлы КАК ЕстьФайлы,
		 |	Дораб_Доставка.Товары.(
		 |		Ссылка КАК Ссылка,
		 |		НомерСтроки КАК НомерСтроки,
		 |		Номенклатура КАК Номенклатура,
		 ...
		 |	Дораб_Доставка.Контрагент.КонтактнаяИнформация.(
